...
public class AgendaDAO {
    ...
    public List<Agenda> readPrenotazioniBeforeToday(Date data) throws SQLException, ClassNotFoundException {
        String query = "SELECT p.data, p.orario, p.chipAnimale, p.chipAnimale, " +
                "s.nomeStato, " +
                "a.nome AS nomeAnimale, " +
                "u.nome, u.cognome " +
        ...
        List<entity.Agenda> prenotazioni = new ArrayList<>();
        ...
        try (Connection conn = DBConnectionManager.getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    entity.Agenda result = new entity.Agenda(prenotazioneData, orario, nomeStato, chipAnimale, nomeAnimale, nomeProprietario);
                    prenotazioni.add(result);
                }
            }catch (SQLException e) {
                throw new SQLException("Errore durante la lettura delle prenotazioni: " + e.getMessage(), e);
